...
Процедура ЗаполнитьТабличнуюЧасть()
	...
	Если ЗначениеЗаполнено(СтруктураДанныеОтпуска) Тогда
		...
		Если СтруктураДанныеОтпуска.ДатаОкончания > КонецМесяца(Объект.Дата) Тогда
			Элементы.ОсновныеНачисления.ДобавитьСтроку(); 
			СтрокаТЧ = Элементы.ОсновныеНачисления.ТекущиеДанные;
			...
			СтрокаТЧ.Сотрудник = СтруктураДанныеОтпуска.Сотрудник; 
			СтрокаТЧ.Подразделение = СтруктураДанныеОтпуска.Подразделение;
			СтрокаТЧ.ДатаНачала = НачалоМесяца(СтруктураДанныеОтпуска.ДатаОкончания); 
...
Функция ДанныеПоМесяцуНачисленияНаСервере(Дата, Ссылка)
	...
	Запрос.Текст = 
	    ...
	    |			И (ВКМ_ГрафикОтпусков.ДатаНачала >= НАЧАЛОПЕРИОДА(&Дата, МЕСЯЦ))
	    |			И (ВКМ_ГрафикОтпусков.ДатаОкончания <= КОНЕЦПЕРИОДА(ДОБАВИТЬКДАТЕ(&Дата, МЕСЯЦ, 1), МЕСЯЦ))
	    |;
	    |
	    |////////////////////////////////////////////////////////////////////////////////
	    |ВЫБРАТЬ
...
Функция КоличествоРабочихЧасовВМесяце(НачалоПериода, КонецПериода)
    ...
    Если КолвоЧасов = 0 Тогда    	
    	ОбщегоНазначения.СообщитьПользователю("Не заполнен 'График работы' для этого периода",
    		, , , 
    		Истина);
